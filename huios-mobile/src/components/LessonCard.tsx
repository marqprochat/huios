...
export function LessonCard({ lesson }: Props) {
  ...
  return <View className="mb-3 rounded-card border border-slate-200 bg-surface p-4 shadow-card">
    ...
    <View className="mt-1 flex-row items-start gap-1.5"><AppIcon name="schedule" accessibilityLabel="Horário" color="#64748b" size={16} /><Text className="flex-1 text-xs text-slate-500">{new Date(lesson.date).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })} • {lesson.startTime ?? 'Horário não informado'}{lesson.endTime ? ` – ${lesson.endTime}` : ''}</Text></View>
