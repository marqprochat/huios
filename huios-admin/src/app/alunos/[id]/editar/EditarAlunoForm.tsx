...
export default function EditarAlunoForm({ aluno, classes, enrollmentsMap }: { aluno: any, classes: any[], enrollmentsMap: Record<string, string> }) {
    const updateAlunoWithId = updateAluno.bind(null, aluno.id);
    const [state, action] = useActionState(updateAlunoWithId, { success: false, message: '' });
    const { toast } = useToast();
    const router = useRouter();

    const birthDateStr = aluno.birthDate ? new Date(aluno.birthDate).toISOString().split('T')[0] : '';
                        ...
                        {classes.map(cls => (
                            <div key={cls.id} className="flex flex-col gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-primary/50 hover:bg-primary/5 transition-all group">
                                <label className="flex items-start gap-3 cursor-pointer">
                                    <input type="checkbox" name="classIds" value={cls.id} defaultChecked={!!enrollmentsMap[cls.id]} className="mt-0.5 w-4 h-4 text-primary border-slate-300 rounded focus:ring-primary" />
                                ...
                                <div className="pl-7">
                                    ...
                                    <select name={`status_${cls.id}`} defaultValue={enrollmentsMap[cls.id] || 'CURSANDO'} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/50 outline-none transition-all dark:text-white">
