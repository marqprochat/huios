...
export function ContasPagarClient({ transactions: initial, categories, teachers }: Props) {
  ...
  const handleExport = () => {
    exportCSV('contas-pagar',
      ['Descrição', 'Categoria', 'Professor', 'Valor', 'Vencimento', 'Status', 'Pago Em'],
      filtered.map(t => [
        t.description,
        t.category?.name ?? '—',
        t.teacher?.name ?? '—',
    ...
    <div className="max-w-[1400px] mx-auto p-4 lg:p-8 space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Contas a Pagar</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm">Despesas e pagamentos da instituição</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'A Pagar', value: totals.pendente, color: 'text-amber-600' },
          { label: 'Pago', value: totals.pago, color: 'text-emerald-600' },
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input type="text" placeholder="Descrição ou professor..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          <span className="material-symbols-outlined text-5xl mb-3">arrow_upward</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[600px]">
              <thead className="bg-slate-50 dark:bg-slate-800/50">
                <tr>
                  {['Descrição', 'Categoria', 'Beneficiário', 'Valor', 'Vencimento', 'Status', 'Ações'].map(h => (
                ...
                {filtered.map(t => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-sm text-slate-600 dark:text-slate-400">{t.teacher?.name ?? '—'}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1">
                          {(t.status === 'PENDENTE' || t.status === 'VENCIDO') && (
                            <button onClick={() => handleMarkPaid(t.id)} disabled={isPending} title="Marcar como pago"
